    ...
    <div className="my-6 overflow-hidden border border-neutral-800 bg-charcoal-900 shadow-2xl">
      ...
      <div className="flex items-center justify-between border-b border-neutral-800 bg-black px-4 py-2.5">
        <div className="flex items-center gap-2">
          ...
          <div className="h-3 w-3 border border-neutral-700 bg-neutral-800"></div>
          <div className="h-3 w-3 border border-neutral-700 bg-neutral-800"></div>
          <span className="ml-4 font-mono text-xs text-neutral-500">{displayFilename}</span>
        </div>
        <button
          onClick={handleCopy}
          disabled={!code}
          className="flex items-center gap-1.5 border border-neutral-800 bg-neutral-900 px-2.5 py-1 text-xs font-medium text-neutral-400 transition-colors hover:border-neutral-700 hover:bg-neutral-800 hover:text-white disabled:cursor-not-allowed disabled:opacity-50"
...
export const InlineCode: React.FC<InlineCodeProps> = ({ children }) => {
  ...
  return (
    <code className="rounded bg-neutral-900 px-1.5 py-0.5 font-mono text-xs text-white">
      {children}
    </code>
  );
};
