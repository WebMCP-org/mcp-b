...
/**
 * Extract sourceId from tool (if available)
 * Tools from WebMCP have _sourceId attached
 */
function getToolSourceId(toolName: string, tools?: Array<{ name: string; _sourceId?: string }>): string | undefined {
  if (!tools) return undefined;
  const tool = tools.find((t) => t.name === toolName);
  return tool ? (tool as typeof tool & { _sourceId?: string })._sourceId : undefined;
}
...
export interface ToolFallbackProps {
  ...
  isError?: boolean;
  /** Optional array of tools to lookup sourceId */
  tools?: Array<{ name: string; _sourceId?: string }>;
}

/**
 * Tool Fallback Component - renders AI-initiated tool calls in chat messages
 *
 * Features:
 * - Collapsible card with parameters and results
 * - Auto-expands on error or during execution
 * - Shows tool source badge (Remote MCP vs WebMCP)
 * - Color-coded status badges
 * - Formatted result display with collapsible full response
 *
 * This is used by assistant-ui to render tool calls in the conversation.
 * Attach it to Thread via `tools={{ fallback: ToolFallback }}`
 */
export const ToolFallback: ToolCallMessagePartComponent = ({
  ...
  // Note: We can't access tools context here directly since this is rendered by assistant-ui
  // Tools array would need to be passed via context or props if sourceId lookup is needed
  const sourceId = undefined; // Will be undefined for now, can be enhanced later with context
